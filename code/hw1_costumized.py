...
def main():
    ...
    image1 = cv2.cvtColor(cv2.imread(os.path.join(main_path, 'data', 'customized.bmp')), cv2.COLOR_BGR2RGB)
    ...
    #print(image1_h, image1_w)
    
    (image2_h, image2_w) = image2.shape[:2]
    #print(image2_h, image2_w)
    ...
    #low_frequencies_array=low_frequencies[0:image1_h,0:image1_w]
    #high_frequencies_array=high_frequencies[0:image2_h,0:image2_w]

    high_frequencies_re = cv2.resize(high_frequencies, (image1_w, image1_h), interpolation=cv2.INTER_LINEAR) #讓兩張圖一樣大
    
    hybrid_image = low_frequencies + high_frequencies_re
